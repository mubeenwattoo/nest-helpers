...
async function submitToGoogleSheetsIncremental(pageData, pageName) {
    ...
    // Helper function to format date as month/day (e.g., 11/23)
    const formatDate = () => {
        const now = new Date();
    ...
    try {
        ...
        if (storedRecordId) {
            ...
            try {
                ...
                if (verifyResponse.ok) {
                    ...
                    console.log(`✅ Found existing record using stored record ID: ${storedRecordId}`);
                } else {
        ...
        if (!existingRecordId && email) {
            ...
            if (searchResponse.ok) {
                ...
                if (searchResult.records && searchResult.records.length > 0) {
                    ...
                    console.log(`✅ Found existing record with email: ${email}`);
                }
            }
        }
        ...
        if (existingRecordId) {
            ...
            if (updateResponse.ok) {
                ...
                console.log(`✅ Data from ${pageName} updated successfully in Airtable`);
                console.log('Updated record:', result);
                return { success: true, response: result, action: 'updated' };
            } else {
                const errorText = await updateResponse.text();
                console.error(`❌ Error updating Airtable record. Status: ${updateResponse.status}`);
            ...
            if (createResponse.ok) {
                ...
                console.log(`✅ Data from ${pageName} created successfully in Airtable`);
                console.log('Created record:', result);
                return { success: true, response: result, action: 'created' };
            } else {
                const errorText = await createResponse.text();
                console.error(`❌ Error creating Airtable record. Status: ${createResponse.status}`);
                console.error(`Error details:`, errorText);
                try {
                    const errorJson = JSON.parse(errorText);
                    console.error('Parsed error:', errorJson);
        ...
        console.error(`❌ Network error submitting data to Airtable from ${pageName}:`, error);
